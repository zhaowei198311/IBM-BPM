        ...
        if (formUid != null && formUid != "") {
            $.ajax({
                ...
                success: function (result) {
                    if (result.status == 0) {
                        var jsIndex = result.data.publicFormContent.indexOf("<script type='text/javascript'>");
                        var demoHtml = "";
                        if (jsIndex != -1) {
                            formatJs = result.data.publicFormContent.substr(jsIndex);
                            demoHtml = result.data.publicFormContent.substring(0, jsIndex);
                        } else {
                            demoHtml = result.data.publicFormContent;
                        }
                        $(".demo").html(demoHtml);
                        var nameVal = "";
                        $(".demo .subDiv").each(function () {
                            ...
                            if (result.status == 0) {
                                for (var i = 0; i < subDivArr.length; i++) {
                                    ...
                                    switch (subObj.prop("tagName")) {
                                        case "INPUT":
                                            {
                                                filedAttr.fldCodeName = formCode+"_"+subObj.attr("name");
                                        ...
                                        case "LABEL":
                                            { //多选框、单选框
                                                filedAttr.fldCodeName = formCode+"_"+subObj.find("input").attr("name");
                                                filedAttr.fldName = subDivObj.prev().find("label").text();
