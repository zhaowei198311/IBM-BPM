$(document).ready(function () {
	...
	if(actcAssignType!='allUser' && actcChooseableHandlerType!='allUser'){
		$("#jstree1").parent().css("display","none");
		$("#jstree1").parent().next().attr("class","col-xs-12");
		$("#jstree1").parent().next().css({"padding-left":"10px","padding-right":"29px"});
	}
			...
			success: function (result){
				if(result.status==0){
					$(result.data).each(function(index){
						$('#users #usersul').append("<li id='" + this.userUid + "' name='" 
		            			+ this.userName + "' title='"+this.station
		            			+"' onClick='selectClick(this);' ondblclick='dblSelClick(this)'>" 
		            			+ this.userName+"("+this.userUid+")" + "</li>");
					});
				}
			}
		});	
