		...
		generate: function(canvas, textMenu)
		{
			...
			$.each($canvas.settings.menu, function(i, item)
			{
				switch(item)
				{
					...
					case 'refresh': menuContent.append($('<div class="_wPaint_icon _wPaint_refresh" title ="'+ $canvas.settings.menuTitles.refresh +'"></div>').click(function(){if(img!=null){$("#wPaint").wPaint("image", img.src);}})); break;
					case 'save': menuContent.append($('<div class="_wPaint_icon _wPaint_save" title ="'+ $canvas.settings.menuTitles.save +'"></div>').click(function(){upload_image();})); break;
				}
			});
