...
public class BpmInstanceUtils extends BpmRestApiUtils {
	...
	/*
	 * public HttpReturnStatus getInstance(HttpServletRequest request, String
	 * instanceId) { return this._getInstance(request, instanceId, false); }
	 */
	...
	private HttpReturnStatus _getInstance(Object webobj, String instanceId, boolean readCache) {
		...
		try {
			...
			if (!readCache) {
				...
				// request = (HttpServletRequest)webobj;
				// String key = (this.clientUtils.isAdmin() ? "admin" : "user") + "_instance_" +
				// instanceId;
	...
	/*
	 * public Map<String, BpmInstanceToken> getTokenIdWithInstance(String
	 * instanceDetails, String taskId) { Map<String, BpmInstanceToken> bpmTokens =
	 * new HashMap(); String instanceId = "";
	 * 
	 * try { if (StringUtils.isNotBlank(instanceDetails)) { JSONObject jsoMsg =
	 * JSONObject.parseObject(instanceDetails); JSONObject jsoData =
	 * jsoMsg.getJSONObject("data"); if (jsoData != null) { instanceId =
	 * jsoData.getString("piid"); JSONObject jsoTreeRoot =
	 * jsoData.getJSONObject("executionTree").getJSONObject("root"); if (jsoTreeRoot
	 * != null) { JSONArray jayChildren = jsoTreeRoot.getJSONArray("children"); if
	 * (jayChildren != null && !jayChildren.isEmpty()) { for(int i = 0; i <
	 * jayChildren.size(); ++i) { JSONObject jsoChild =
	 * jayChildren.getJSONObject(i); if (jsoChild != null) { String rootTokenId =
	 * jsoChild.getString("tokenId"); String rootFlowObjectId =
	 * jsoChild.getString("flowObjectId"); BpmInstanceToken rootToken = new
	 * BpmInstanceToken(); rootToken.setTokenId(rootTokenId);
	 * rootToken.setRootTokenId(rootTokenId);
	 * rootToken.setActivityBpdId(rootFlowObjectId); bpmTokens.put("root",
	 * rootToken); BpmInstanceToken taskToken = this.getTokenIdWithToken(jsoChild,
	 * taskId); if (taskToken != null) { taskToken.setRootTokenId(rootTokenId);
	 * bpmTokens.put("task", taskToken); break; }
	 * 
	 * bpmTokens = new HashMap(); } } } } } } } catch (Exception var15) {
	 * LOG.error("获取实例Token ID出错，instanceId：" + instanceId + ", taskId：" + taskId,
	 * var15); }
	 * 
	 * return bpmTokens; }
	 */
	 ...
	 * List<BpmInstanceToken> tokens = new ArrayList(); String instanceId = "";
	 * 
	 * try { if (StringUtils.isNotBlank(instanceDetails)) { JSONObject jsoMsg =
	 * JSONObject.parseObject(instanceDetails); JSONObject jsoData =
	 * jsoMsg.getJSONObject("data"); if (jsoData != null) { instanceId =
	 ...
	 * (jayChildren != null && !jayChildren.isEmpty()) { for(int i = 0; i <
	 * jayChildren.size(); ++i) { JSONObject jsoChild =
	 * jayChildren.getJSONObject(i); BpmInstanceToken instToken = new
	 * BpmInstanceToken(); if (jsoChild != null) {
	 * instToken.setActivityBpdId(jsoChild.getString("flowObjectId"));
	 * instToken.setActivityName(jsoChild.getString("name"));
	 * instToken.setInstanceId(instanceId); String tokenId =
	 * jsoChild.getString("tokenId"); instToken.setTokenId(tokenId);
	 * instToken.setRootTokenId(tokenId); JSONArray taskIds =
	 * jsoChild.getJSONArray("createdTaskIDs"); Map<String, String> tokenTaskIds =
	 * new HashMap(); if (taskIds != null && !taskIds.isEmpty()) {
	 * tokenTaskIds.put(tokenId, taskIds.getString(0)); } else { JSONArray
	 * jaySubChildren = jsoChild.getJSONArray("children"); if (jaySubChildren !=
	 * null) { for(int j = 0; j < jaySubChildren.size(); ++j) { JSONObject
	 * jsoSubChild = jaySubChildren.getJSONObject(j); if (jsoSubChild != null) {
	 * tokenTaskIds.putAll(this.findTokenTasks(jsoSubChild)); } } } }
	 * 
	 * instToken.setTaskIds(tokenTaskIds); tokens.add(instToken); } } } } } } }
	 * catch (Exception var17) { LOG.error("获取实例Token列表时发生异常，instanceId：" +
	...
	public HttpReturnStatus getAllInstanceByName(HttpServletRequest request, String instanceName, int offset,
			int rows) {
		HttpReturnStatus instHttpStatus = new HttpReturnStatus();
	...
	public HttpReturnStatus moveToken(HttpServletRequest request, String tokenId, String instanceId,
			String targetNodeId) {
	...
	/*
	 * public Map<String, HttpReturnStatus> setInstancePubBo(HttpServletRequest
	 * request, String instanceId, BpmCommonBusObject pubBo) { Map<String,
	 * HttpReturnStatus> httpStatusMap = new HashMap(); HttpReturnStatus
	 * instHttpStatus = this.getInstanceData(request, instanceId);
	 * httpStatusMap.put("getInstanceDataResult", instHttpStatus); if
	 * (!BpmClientUtils.isErrorResult(instHttpStatus)) { org.json.JSONObject jsoMsg
	 * = new org.json.JSONObject(instHttpStatus.getMsg()); org.json.JSONObject
	 * jsoVar = jsoMsg.getJSONObject("data").getJSONObject("variables");
	 * org.json.JSONObject jsoBo = jsoVar.optJSONObject("pubBo"); if (jsoBo != null)
	 * { if (jsoBo.get("@metadata") != null) { jsoBo.remove("@metadata"); }
	 * 
	 * org.json.JSONObject jsoNew = new org.json.JSONObject(pubBo); jsoNew =
	 * JSONUtils.combine(jsoNew, jsoBo); String host =
	 * this.bpmGlobalConfig.getBpmServerHost(); host = host.endsWith("/") ? host :
	 * host + "/"; String url = host + "rest/bpm/wle/v1/process/{0}" +
	 * "/variable/pubBo"; url = MessageFormat.format(url, instanceId);
	 * HttpReturnStatus httpStatus = this.clientUtils.doPut(request, url,
	 * jsoNew.toString()); httpStatusMap.put("setInstanceBOMsg", httpStatus); if
	 * (BpmClientUtils.isErrorResult(httpStatus)) { LOG.error("设置流程实例BO对象失败！");
	 * httpStatusMap.put("errorResult", httpStatus); } } else {
	 * LOG.error("pubBo对象为空！"); HttpReturnStatus errorStatus = new
	 * HttpReturnStatus(); errorStatus.setCode(-1); JSONObject msg = new
	 * JSONObject(); msg.put("msg", "pubBo对象为空！");
	 * errorStatus.setMsg(msg.toJSONString()); httpStatusMap.put("errorResult",
	 * errorStatus); } } else { LOG.error("获取流程实例数据信息失败！");
	...
	/*
	 * public BpmInstanceToken getTokenIdWithToken(JSONObject tokenDetail, String
	 * taskId) { BpmInstanceToken bpmToken = null; JSONArray jayTaskIds =
	 * tokenDetail.getJSONArray("createdTaskIDs"); JSONArray jayChildren =
	 * tokenDetail.getJSONArray("children"); if (jayTaskIds != null &&
	 * !jayTaskIds.isEmpty()) { String tmpTaskId = jayTaskIds.getString(0); if
	 * (taskId.equals(tmpTaskId)) { bpmToken = new BpmInstanceToken(); String
	 * tokenId = tokenDetail.getString("tokenId"); bpmToken.setTokenId(tokenId);
	 * bpmToken.setActivityBpdId(tokenDetail.getString("flowObjectId")); Map<String,
	 * String> taskIds = new HashMap(); taskIds.put(tokenId, taskId);
	 * bpmToken.setTaskIds(taskIds); } } else if (jayChildren != null &&
	 * !jayChildren.isEmpty()) { for(int i = 0; i < jayChildren.size(); ++i) {
	 * JSONObject jsoChild = jayChildren.getJSONObject(i); bpmToken =
	 * this.getTokenIdWithToken(jsoChild, taskId); if (bpmToken != null) { break; }
	 * } }
	 * 
	 * return bpmToken; }
	 */
	...
	public String extractErrorMsg(String errMsg) {
		...
		if (StringUtils.isNotBlank(errMsg)) {
			try {
				...
				result = tmpError[1].substring(tmpError[1].indexOf("//") + 2,
						tmpError[1].indexOf("/", tmpError[1].indexOf("//") + 2));
